...
export default function HeadToHead() {
  ...
  useEffect(() => {
    async function loadData() {
      const [playersRes, atBatsRes] = await Promise.all([
    ...
    <div className="min-h-screen pb-24" style={{ background: '#080D18' }}>
      ...
      >
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-[#4A5772]">Compare player stats</div>
        </div>
      </div>

      <div className="max-w-2xl mx-auto p-5 space-y-6">
        {/* Player selectors */}
        <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible" className="flex gap-4">
          <div className="flex-1">
            <PlayerSelector
              players={players}
              selected={player1Id}
        ...
        {winner && stats1 && stats2 && (
          ...
          >
            <Trophy size={18} color="#F0B429" />
            <span className="text-sm font-semibold text-[#F0B429]">
              {winner.name} leads {advantages.p1 > advantages.p2 ? advantages.p1 : advantages.p2}-
